function [unmix, gmap] = ismrm_calculate_sense_unmixing_general(acc_factor, csm, noise_matrix, regularization_factor)
%
...
for ix_blocks=1:nx_blocks
    for iy_blocks = 1:ny_blocks
        ...
        if max(abs(A(:))) > 0,
            ...
            unmix2d(ix_blocks:nx_blocks:nx,iy_blocks:ny_blocks:ny, :) = reshape(pinv(AHA + reduced_eye .* scaled_reg_factor) * A' * noise_matrix_inv,acc_factor_x,acc_factor_y,[]);
        end
    end
end
